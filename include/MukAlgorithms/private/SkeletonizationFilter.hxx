...
namespace itk
{
  ...
  void SkeletonizationFilter<TInputImage, TOutputImage>::GenerateData()
  {
        ...
        for(unsigned int x=1; x<size[0]-1; ++x)
    {
      ...
      if(theval != 0)
      {
        ...
        */
        ...
        else // Find direction of max curvature
        {
        ...
        for (unsigned int x = 1; x<size[0]-1; ++x)
    {
      typename InputImageType::IndexType ix;
      ix[0] = x; ix[1] = y; ix[2] = z;
      auto vec = m_DirectionImage->GetPixel(ix);
      if ( ! (vec[0] == 0 && vec[1] == 0 && vec[2] == 0))
        ++sum;
    }
    std::cout << "numel direction image: " << sum << std::endl;
    ...
    std::cout << "numel direction image 2: " << sum << std::endl;
    ...
    for(unsigned int z=1; z<size[2]-1; ++z)
      for(unsigned int y=1; y<size[1]-1; ++y)
        for (unsigned int x = 1; x<size[0]-1; ++x)
        {
          typename InputImageType::IndexType ix;
          ix[0] = x; ix[1] = y; ix[2] = z;
          auto val = outputImg->GetPixel(ix);
          if (val != 0)
            ++sum;
        }

    std::cout << "numel output image: " << sum << std::endl;
