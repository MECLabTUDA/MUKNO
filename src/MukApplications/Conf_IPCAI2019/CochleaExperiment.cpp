...
namespace
{
  ...
  */
  class CochleaExperiment : public Experiment
  {
    public:
      CochleaExperiment(const ExperimentInput& input)
        : Experiment(input, enCochlear)
      {
      }
...
namespace muk
{
  /**
  */
  void runCochleaExperiment(const ProgramInput& programInput)
  {
    auto& input = programInput.cochleaData;
    // created result output directories;
    auto pExp = std::make_unique<CochleaExperiment>(input);
    pExp->initialize();
    pExp->run();
    // save log file for debugging
    fs::copy_file( gris::GetGrisLogger().getFilename(), fs::path(input.outputDir) / ScenarioNames[enCochlear] / input.lastEvalDate / "log_run.txt", fs::copy_option::overwrite_if_exists);
  }

  /**
  */
  void evaluateCochleaExperiment(const ProgramInput& programInput)
  {
    const auto& input = programInput.cochleaData;
    {
      auto pExp = std::make_unique<CochleaExperiment>(input);
      pExp->eval();
    }
    const auto newFile = fs::path(input.outputDir) / ScenarioNames[enCochlear] / input.lastEvalDate / "log_eval.txt";
    fs::copy_file( gris::GetGrisLogger().getFilename(), newFile, fs::copy_option::overwrite_if_exists);
  }
}
}
...
namespace
{
  /**
  */
  void CochleaExperiment::runExperiment(const PatientData& patient)
  {
    LOG_LINE << "=== planning cochlear access " << patient.id << " ==========";
    ...
    const auto& input  = static_cast<const CochleaExperimentInput&>(mInput);
    ...
    for (const auto& canal : config.accessCanals)
    {
      ...
      }
      {
        ...
        {
          size_t numPaths = 1u;
          plan.createPaths(canal, numPaths);
        }
      ...
      try
      {
        LOG_LINE << "   Loading manual segmentations from : " << (fs::path(config.segmentationDirManual) / patient.id).string();
      ...
      try
      {
        LOG_LINE << "   Loading auto segmentations from : " << (fs::path(config.segmentationDirAuto) / patient.id).string();
        // first change the data in the collision detector
        for (size_t i(0); i<organs.size(); ++i)
        {
  ...
  /**
  */
  void CochleaExperiment::evalExperiment(const PatientData& patient)
  {
    const auto rootDir = fs::path(mInput.outputDir) / ScenarioNames[enCochlear] / mInput.lastEvalDate / patient.id;
    ...
    for (const auto& canal : config.accessCanals)
    {
      ...
      // save the scene
      auto sceneFile = rootDir / ( patient.id + "_scene.mukscene");
  ...
  /**
  */
  void CochleaExperiment::evalExperiments()
  {
    const auto rootDir = fs::path(mInput.outputDir) / ScenarioNames[enCochlear] / mInput.lastEvalDate;
    ...
    for (const auto& canal : config.accessCanals)
    {
      ...
      const double safetyDist = 0.3; // TODO read that in somewhere
      ...
      const auto gtMean             = std::accumulate(data.minDistGt.begin(), data.minDistGt.end(), 0.0, [&] (double in, double d) { return in + 0.5 + std::max(0.0,d); }) / (1.0*N_GT);
      const auto manualMean         = std::accumulate(data.minDistMa.begin(), data.minDistMa.end(), 0.0, [&] (double in, double d) { return in + 0.5 + std::max(0.0,d); }) / (1.0*N_Manual);
      const auto autoMean           = std::accumulate(data.minDistAu.begin(), data.minDistAu.end(), 0.0, [&] (double in, double d) { return in + 0.5 + std::max(0.0,d); }) / (1.0*N_Auto);
