  ...
  namespace muk
  {
    ...
    */
    void fitSphere(const Vec3d& p1, const Vec3d& p2, const Vec3d& p3, Sphere3D& output)
    {
      ...
      {
        LOG_LINE << "Warning: Some points are equal." << (boost::format("p1=%s, p2=%s p3=%s)") % p1 % p2 % p3).str().c_str();
        output.setCenter(p2);
        output.setRadius(std::min( (p1-p2).norm(), (p3-p2).norm()) );
      }
      else
      {
        using namespace Eigen;
    ...
    */
    ...
    /**
    */
    void Line3D::intersects(const Line3D& line, Query& query)
    {
      using namespace Eigen;
