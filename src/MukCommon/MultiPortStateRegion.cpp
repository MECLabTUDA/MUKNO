...
#include "MultiPortStateRegion.h"
...
BOOST_CLASS_EXPORT(gris::muk::MultiPortStateRegion);

namespace gris
{
  namespace muk
  {
    REGISTER_REGION(MultiPortStateRegion);

    /**
    */
    MultiPortStateRegion::MultiPortStateRegion()
      : mRadius(1)
    ...
    /**
    */
    void MultiPortStateRegion::setPhi(double val)
    {
    ...
    */
    std::vector<MukState> MultiPortStateRegion::getStates() const
    {
      std::vector<MukState> result;
      LOG_LINE << "N: " << mStates.size();
      for (size_t i(0); i<mStates.size(); ++i)
      {
        using namespace Eigen;
        Transform<double, 3, 1> R;
        auto& center = mStates[i];
        {
          auto zAxis = Vec3d(0, 0, 1);
          auto crossproduct = zAxis.cross(center.tangent);
          ...
          auto tr = Translation3d(Vector3d(cast(center.coords)));
        }
        ...
        for (size_t i(0); i<=mResolution; ++i, phi += stepPhi) // for each latitude ...
        {
          ...
          for (size_t j(0); j<=N; ++j) // ... compute states on the circle (longitude)
          {
            ...
            nextState.coords = center.coords;
      ...
      LOG_LINE << "N2: " << result.size();
      return result;
    }

    /**
    */
    void MultiPortStateRegion::setState(size_t idx, const MukState& s)
    {
      mStates[idx] = s;
    }
    ...
    /**
    */
    std::unique_ptr<IStateRegion> MultiPortStateRegion::clone() const
    {
      auto pObj = std::make_unique<MultiPortStateRegion>();
      pObj->mStates     = mStates;
      pObj->mRadius     = mRadius;
      pObj->mPhi        = mPhi;
    ...
    */
    template<>
    void MultiPortStateRegion::serialize<boost::archive::text_iarchive>(boost::archive::text_iarchive& ar, const unsigned version)
    {
      ...
      boost::serialization::void_cast_register<MultiPortStateRegion, IStateRegion>();
      //ar & boost::serialization::base_object<IStateRegion>(*this); // instead of this
      ar & mStates & mRadius & mPhi & mResolution;
    }

    /**
    */
    template<>
    void MultiPortStateRegion::serialize<boost::archive::text_oarchive>(boost::archive::text_oarchive& ar, const unsigned version)
    {
      boost::serialization::void_cast_register<MultiPortStateRegion, IStateRegion>();
      ar & mStates & mRadius & mPhi & mResolution;
    }
